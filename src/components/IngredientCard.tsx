...
interface IngredientCardProps {
  ...
  onUpdate: (id: string, data: { name: string; defaultUnit: string; description: string }) => Promise<void>;
}

export default function IngredientCard({ ingredient, onDelete, onUpdate }: IngredientCardProps) {
  ...
  const handleConfirmDelete = async (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    setIsDeleting(true);
    const success = await onDelete(ingredient._id);
    if (!success) {
      setIsDeleting(false);
  ...
  const handleSave = async (e: React.MouseEvent) => {
    ...
    setIsUpdating(true);
    try {
      await onUpdate(ingredient._id, editData);
      setIsEditing(false);
    } finally {
      setIsUpdating(false);
    }
    ...
    <div className={`relative p-4 border rounded-lg hover:shadow-md transition-shadow group ${isDeleting ? 'scale-95 opacity-0' : ''}`}>
      {!isEditing ? (
        <>
          ...
          <p className="text-gray-600 text-sm">Default Unit: {ingredient.defaultUnit}</p>
          <p className="text-gray-800 text-sm">{ingredient.description}</p>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => setEditData({ ...editData, defaultUnit: e.target.value as "g" | "kg" | "ml" | "l" | "piece" | "tbsp" | "tsp" | "cup" })}
            className="w-full p-1 text-sm border rounded focus:ring-1 focus:ring-primary-500 focus:border-primary-500 text-gray-900"
          >
